...
dataLayers = {"City Limits Fresno":{"file":"City Limits Fresno County","layer":null},"CUSD Boundary":{"file":"CUSD Boundary","layer":null},"CUSD Current Trustee Areas":{"file":"CUSD Current Trustee Areas","layer":null}   }
drawingLayer = "layers/county.geojson";
colors1 = ["#E41A1C","#377EB8","#4DAF4A","#984EA3","#FF7F00","#FFFF33","#999999","#ffffff"];
   ...
   layertimer = setInterval(function(){
      if (getdatalayers){
          ...
          shp("./layers/"+dataLayers[dlayers[datalayersstart]]["file"]).then(function(geojson){  
              ...
              if(dlayers[datalayersstart] == 'CUSD Current Trustee Areas'){
                  div.append(br)
                  img = document.createElement("img");
                  img.src = '../../static/images/cusd_dist.png'
                  img.width = 150
                  div.append(img);
              }else if(dlayers[datalayersstart] == 'City Limits Fresno' ){
                  div.append(br)
                  img = document.createElement("img");
                  img.src = '../../static/images/county.png'
                  img.width = 40
                  div.append(img);
              }else if(dlayers[datalayersstart] == 'CUSD Boundary' ){
                div.append(br)
                img = document.createElement("img");
                img.src = '../../static/images/cusd_boundary.png'
                img.width = 60
                div.append(img);
            }   
...
function generateLink(){
  ...
  ref = firebase.database().ref('Users/cusd/'+file)
  filefound = false;
  ...
  setTimeout(function(){
     if (!filefound){
         ...
         firebase.database().ref('Users/cusd/' + file).set(ggeojson);
...
function addlayer(title,geojson,color,visible){
       ...
       if (title == 'CUSD Current Trustee Areas'){
        map.addLayer({
            'id': title,
            'type': 'fill',
            'source': title,
        ...
        else if (title == 'City Limits Fresno'){
            map.addLayer({
                'id': title,
                'type': 'line',
                'source': title,
...
if (nlink.split("?id=")[1]){
   ...
   fetch(drawingLayer)
     ...
     .then((data) => {
        ...
        ref = firebase.database().ref('Users/cusd/' + uuid);
